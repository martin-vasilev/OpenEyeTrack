...
export class CalibrationController {
  ...
  constructor(private readonly target:HTMLElement,config:CalibrationConfig,private readonly sessionData?:SessionDataManager){this.config=cloneConfig(config);this.model=createGazeEstimator(this.config.model);this.accumulatedModelKey=modelKey(this.config.model);this.applyTargetStyle();}
  ...
  async recalibrateTargets(getFeatures:()=>EyeHeadFeatures|null,points:ValidationPointResult[]):Promise<CalibrationSummary>{
    ...
    const newObservations:CalibrationObservation[]=[],calibrationFeatureSamples:EyeHeadFeatures[]=[];let samplesCollected=0;this.target.hidden=false;const calibrationRun=this.sessionData?.beginCalibration(this.config,"targeted")??null;
    try{for(let pointIndex=0;pointIndex<points.length;pointIndex++){const point=points[pointIndex],nx=point.targetX/innerWidth,ny=point.targetY/innerHeight,targetOnsetMs=performance.now();this.place(nx,ny);await wait(this.config.settleMs);const samplingStartMs=performance.now(),timedSamples=await collectFeatureSamples(getFeatures,this.config.sampleMs,this.config.featureModel==="elg"),samplingEndMs=performance.now(),samples=timedSamples.map(s=>s.features);const pointRecord:CalibrationPointRecord={run:calibrationRun??0,round:1,point:pointIndex+1,targetX:point.targetX,targetY:point.targetY,targetXNorm:nx,targetYNorm:ny,targetOnsetMs,samplingStartMs,samplingEndMs};timedSamples.forEach((s,i)=>this.sessionData?.recordCalibrationSample(pointRecord,i+1,s.timestampMs,s.features));calibrationFeatureSamples.push(...samples);samplesCollected+=samples.length;const stable=selectStableCalibrationSamples(samples);for(let i=0;i<stable.length;i++){const features=stable[i];newObservations.push({targetX:point.targetX,targetY:point.targetY,features});this.sessionData?.recordCalibrationFitObservation(pointRecord,i+1,features);}this.sessionData?.recordCalibrationPointSummary(pointRecord,samples.length,stable.length);}}finally{this.target.hidden=true;}
    ...
    this.lastSummary={id:`cal-${Date.now()}`,config:cloneConfig(this.config),observations:combined.length,newObservations:newObservations.length,retainedObservations:retained.length,targetsCompleted:points.length,samplesCollected,adaptiveUsed:true,headPoseReference:buildHeadPoseReference(calibrationFeatureSamples)??this.lastSummary?.headPoseReference??null};if(calibrationRun!==null)this.sessionData?.finishCalibration(calibrationRun,this.lastSummary);return this.lastSummary;
  }
  async validate(getFeatures:()=>EyeHeadFeatures|null):Promise<ValidationResult>{
    const validationPoints=POINT_SETS[this.config.points],results:ValidationPointResult[]=[];let totalExpected=0,totalValid=0;this.target.hidden=false;const validationRun=this.sessionData?.beginValidation(this.sessionData.currentCalibrationRun,"validation")??null;
    try{for(let pointIndex=0;pointIndex<validationPoints.length;pointIndex++){const[nx,ny]=validationPoints[pointIndex],targetX=nx*innerWidth,targetY=ny*innerHeight,targetOnsetMs=performance.now();this.place(nx,ny);await wait(this.config.settleMs);const samplingStartMs=performance.now(),timedSamples=await collectFeatureSamples(getFeatures,this.config.sampleMs,this.config.featureModel==="elg"),samplingEndMs=performance.now(),predictions:GazePrediction[]=[];const expected=Math.max(1,Math.round(this.config.sampleMs/33));const pointRecord:ValidationPointRecord={validationRun:validationRun??0,calibrationRun:this.sessionData?.currentCalibrationRun??null,eventType:"validation",point:pointIndex+1,targetX,targetY,targetXNorm:nx,targetYNorm:ny,targetOnsetMs,samplingStartMs,samplingEndMs};for(let i=0;i<timedSamples.length;i++){const s=timedSamples[i],p=this.model.predict(s.features),valid=p&&Number.isFinite(p.x)&&Number.isFinite(p.y)?p:null;if(valid)predictions.push(valid);this.sessionData?.recordValidationSample(pointRecord,i+1,s.timestampMs,s.features,valid);}totalExpected+=expected;totalValid+=Math.min(expected,predictions.length);if(predictions.length){const x=median(predictions.map(p=>p.x)),y=median(predictions.map(p=>p.y)),pointResult={targetX,targetY,samplesExpected:expected,samplesValid:predictions.length,accuracyPx:Math.hypot(x-targetX,y-targetY),precisionRmsS2SPx:rmsS2S(predictions),precisionSdPx:spatialSd(predictions)};results.push(pointResult);this.sessionData?.recordValidationPointSummary(pointRecord,{samplesExpected:expected,samplesValid:predictions.length,observedX:x,observedY:y,accuracyPx:pointResult.accuracyPx,precisionRmsS2SPx:pointResult.precisionRmsS2SPx,precisionSdPx:pointResult.precisionSdPx});}else this.sessionData?.recordValidationPointSummary(pointRecord,{samplesExpected:expected,samplesValid:0,observedX:null,observedY:null,accuracyPx:null,precisionRmsS2SPx:null,precisionSdPx:null});}}finally{this.target.hidden=true;}
    if(!results.length)throw new Error("No valid gaze samples were collected during validation.");this.memory.save(this.config.model.type,results);const errors=results.map(r=>r.accuracyPx),result={meanPx:mean(errors),medianPx:median(errors),rmsePx:Math.sqrt(mean(errors.map(e=>e*e))),precisionRmsS2SPx:mean(results.map(r=>r.precisionRmsS2SPx)),precisionSdPx:mean(results.map(r=>r.precisionSdPx)),dataLoss:Math.max(0,1-totalValid/totalExpected),points:results.length,pointResults:results};if(validationRun!==null)this.sessionData?.finishValidation(validationRun,result);return result;
  }
...
type TimedFeatureSample={features:EyeHeadFeatures;timestampMs:number};
async function collectFeatureSamples(getFeatures:()=>EyeHeadFeatures|null,sampleMs:number,freshElgOnly:boolean):Promise<TimedFeatureSample[]>{
  const expected=Math.max(1,Math.round(sampleMs/33));
  const samples:TimedFeatureSample[]=[];
  ...
  while(performance.now()<deadline&&samples.length<expected){
    const f=getFeatures();
    if(f){
      if(!freshElgOnly){samples.push({features:{...f},timestampMs:performance.now()});}
      else if(f.elgTimestampMs!==null&&!seenElg.has(f.elgTimestampMs)){
        seenElg.add(f.elgTimestampMs);samples.push({features:{...f},timestampMs:f.elgAcquisitionTimestampMs??performance.now()});
      }
    }
    await wait(8);
  }
