...
export class CalibrationController {
  ...
  async calibrate(getFeatures:()=>EyeHeadFeatures|null):Promise<CalibrationSummary>{
    ...
    try{for(let i=0;i<sequence.length;i++){const[nx,ny]=sequence[i];this.place(nx,ny);this.setHeadPoseCue(i);await wait(this.config.settleMs);const samples=await collectFeatureSamples(getFeatures,this.config.sampleMs,this.config.featureModel==="elg");for(const copy of samples){calibrationFeatureSamples.push(copy);samplesCollected++;}if(samples.length){const stable=selectStableCalibrationSamples(samples);for(const features of stable)newObservations.push({targetX:nx*innerWidth,targetY:ny*innerHeight,features});}}}finally{this.target.hidden=true;this.target.removeAttribute("data-head-pose");}
  ...
  private calibrationSequence(){const sequence:(readonly[number,number])[]=[];for(let r=0;r<this.config.repetitions;r++){let p=this.activePoints.map(([x,y])=>{if(!this.config.jitterTargets)return[x,y]as const;const j=.035;return[clamp(x+(Math.random()*2-1)*j,.07,.93),clamp(y+(Math.random()*2-1)*j,.07,.93)]as const;});if(this.config.randomize)shuffle(p);sequence.push(...p);}return sequence;}
  private setHeadPoseCue(i:number){if(!this.config.headPoseVariation){this.target.removeAttribute("data-head-pose");return;}const cues=["centre","left","right","up","down"];this.target.dataset.headPose=cues[i%cues.length];}
