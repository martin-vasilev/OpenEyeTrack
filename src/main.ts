...
<div id="head-position-screen" class="head-position-screen" hidden><div class="head-position-card"><p class="eyebrow">Before calibration</p><h2>Position your head</h2><p>Sit comfortably and align your face with the template. Keep the screen directly in front of you and use this position during calibration.</p><div class="head-position-view"><video id="head-position-video" autoplay muted playsinline></video><div id="head-position-guide" class="head-position-guide"><div class="head-position-oval"></div><div class="head-position-crosshair"></div></div></div><div id="head-position-message" class="head-position-message">Waiting for face detection…</div><div class="head-position-tips"><span>Face centred</span><span>Comfortable distance</span><span>Both eyes visible</span></div><div class="calibration-actions"><button id="head-position-back">Back</button><button id="head-position-continue" class="primary" disabled>Continue to calibration</button></div></div></div><div id="calibration-setup" class="calibration-screen" hidden><div class="calibration-card">
  ...
  <label class="toggle"><input id="cal-head-pose" type="checkbox" checked /> Add left, right, up and down head-position calibration rounds</label>
...
<div id="demo-screen" class="demo-screen" hidden>
  ...
  <div class="stimulus-tabs" role="tablist"><button data-stimulus="images" class="active">Images</button><button data-stimulus="text">Reading</button><button data-stimulus="video">Videos</button></div>
  <main id="stimulus-area" class="stimulus-area">
    <section class="stimulus-panel active stimulus-carousel" data-panel="images">
    ...
    <section class="stimulus-panel reading-stimulus stimulus-carousel" data-panel="text">
      <button class="stimulus-prev" aria-label="Previous passage">‹</button><article id="demo-reading"></article><button class="stimulus-next" aria-label="Next passage">›</button>
    </section>
  ...
  <p class="recording-indicator"><span></span> Gaze data are being recorded. Use the arrows to browse examples.</p>
</div>
...
let poorValidationTargets:import("./calibration/CalibrationController").ValidationPointResult[]=[],recording=false,lastRecording:EyeTrackingSample[]=[],statusTimer:number|null=null,detectionAnimation:number|null=null,lastProcessedVideoTime=-1,captureWorker:Worker|null=null,captureTrack:MediaStreamTrack|null=null,captureFrames=0,captureDropped=0,captureQueueDepth=0,detectedFaces=0,latestFeatures:EyeHeadFeatures|null=null,latestFace:import("@mediapipe/tasks-vision").NormalizedLandmark[]|null=null,latestAppearance:AppearanceGazeFeatures|null=null,latestElg:ElgEyeFeatures|null=null,calibrationActive=false,headPositionReadySince:number|null=null,headPositionLatched=false;

startButton.onclick=async()=>{try{status.textContent="Loading face landmark model…";await faceTracker.initialize();status.textContent="Requesting camera permission…";const settings=await tracker.start();placeholder.hidden=true;startButton.disabled=true;stopButton.disabled=false;recordButton.disabled=false;calibrateButton.disabled=false;if(!startDecoupledAcquisition())runLandmarks();updateStatus(settings);statusTimer=window.setInterval(()=>updateStatus(settings),500);}catch(e){status.textContent=`Startup error: ${e instanceof Error?e.message:String(e)}`;}};
recordButton.onclick=()=>{if(recording)void finishRecording("recording_stop");else beginRecording();};
stopButton.onclick=()=>{closeHeadPosition();if(recording)void finishRecording("recording_stop");if(statusTimer!==null)clearInterval(statusTimer);if(detectionAnimation!==null)cancelAnimationFrame(detectionAnimation);stopDecoupledAcquisition();faceTracker.close();overlay.clear();tracker.stop();gazeFilter.reset();headGuard.reset();placeholder.hidden=false;startButton.disabled=false;stopButton.disabled=true;recordButton.disabled=true;calibrateButton.disabled=true;validateButton.disabled=true;gazeDot.hidden=true;exportButton.disabled=!lastRecording.length;status.textContent="Camera stopped.";};
...
repeatBadTargets.onclick=async()=>{if(!poorValidationTargets.length)return;validationResults.hidden=true;calibrationStage.hidden=false;calibrationActive=true;gazeDot.hidden=true;setBusy(true);status.textContent=`Repeating ${poorValidationTargets.length} inaccurate validation target${poorValidationTargets.length===1?"":"s"}…`;try{const summary=await calibration.recalibrateTargets(()=>latestFeatures,poorValidationTargets);tracker.setCalibrationSummary(summary);status.textContent=`Targeted recalibration complete: ${poorValidationTargets.length} targets repeated. Re-validating the updated model…`;poorValidationTargets=[];validateButton.click();}catch(e){validationResults.hidden=false;status.textContent=`Targeted recalibration error: ${e instanceof Error?e.message:String(e)}`;}finally{calibrationActive=false;calibrationStage.hidden=true;gazeFilter.reset();setBusy(false);}};
exitDemo.onclick=()=>{void finishDemo();};
...
startCalibrationButton.onclick=async()=>{
  ...
  try{
    ...
    const summary=await calibration.calibrate(()=>latestFeatures,async round=>{if(!config.headPoseVariation||round===0){status.textContent="Calibration round 1 of 5 · normal head position";return;}await positionForCalibrationRound(round);status.textContent=`Calibration round ${round+1} of 5 · ${["","left","right","up","down"][round]} head position`;});
...
function startDecoupledAcquisition():boolean{
  ...
  if(!Processor||typeof VideoFrame==="undefined"){console.warn("[OpenEyeTrack] MediaStreamTrackProcessor unavailable; using legacy frame loop.");return false;}
  try{
    ...
    captureWorker.onmessage=(event:MessageEvent<{type:string;frame?:VideoFrame;captured?:number;dropped?:number;queueDepth?:number}>)=>{
      const msg=event.data;if(msg.type!=="frame"||!msg.frame)return;
      captureFrames=msg.captured??captureFrames;captureDropped=msg.dropped??captureDropped;captureQueueDepth=msg.queueDepth??captureQueueDepth;
      processCapturedFrame(msg.frame);
    };
    const readable=processor.readable;
    captureWorker.postMessage({type:"init",readable,maxQueue:8},[readable as unknown as Transferable]);
    captureWorker.postMessage({type:"next"});
    console.info("[OpenEyeTrack] Decoupled frame acquisition enabled.");
    return true;
  }catch(e){console.warn("[OpenEyeTrack] Could not start decoupled acquisition; using legacy frame loop.",e);stopDecoupledAcquisition();return false;}
}
...
function processCapturedFrame(frame:VideoFrame){
  try{
    ...
    const frameTimestampMs=Number.isFinite(frame.timestamp)?frame.timestamp/1000:detectStarted;
    const result=faceTracker.detectFrame(frame,frameTimestampMs);
    ...
    if(result){
      ...
      if(face){
        latestFace=face;
        if(selected==="elg"){const t=performance.now();void elgTracker.estimateFrame(frame,face).then(v=>{if(v)latestElg=v;});elgEnqueueMs=performance.now()-t;}
        else{void appearanceTracker.estimateFrame(frame,face).then(v=>{if(v)latestAppearance=v;});}
        const matrix=result.facialTransformationMatrixes?.[0]?.data;
        const featureStarted=performance.now();latestFeatures=extractEyeHeadFeatures(face,matrix?Array.from(matrix):undefined);featureExtractionMs=performance.now()-featureStarted;
        ...
        }else if(latestFeatures&&selected!=="mediapipe"&&latestAppearance){latestFeatures.appearanceGazeYaw=latestAppearance.yaw;latestFeatures.appearanceGazePitch=latestAppearance.pitch;}
        ...
        const gazeStarted=performance.now();const raw=latestFeatures?calibration.model.predict(latestFeatures):null,filterResult=raw?gazeFilter.filter(raw):null,filtered=filterResult?.gaze??null;gazePredictionMs=performance.now()-gazeStarted;
      ...
      }else{latestFeatures=null;tracker.setFeatures(null);tracker.setGaze(null);gazeFilter.reset();gazeDot.hidden=true;}
    }
...
function openDemo(){demoScreen.hidden=false;gazeDot.hidden=false;setStep("demo");renderDemoItem("images");renderDemoItem("text");renderDemoItem("video");selectStimulus("images");if(recording)void finishRecording("recording_stop");beginRecording();tracker.setTrial("demo-images-1");tracker.mark("demo_start");}
...
async function finishDemo(){if(recording)await finishRecording("demo_end");demoScreen.hidden=true;gazeDot.hidden=true;demoResults.hidden=false;demoSampleCount.textContent=`${lastRecording.length} samples recorded`;heatmapFilter.value="all";renderHeatmap("all");}
...
const demoImage=q<HTMLImageElement>("#demo-image"),demoImageCaption=q<HTMLElement>("#demo-image-caption"),demoReading=q<HTMLElement>("#demo-reading"),demoVideo=q<HTMLVideoElement>("#demo-video"),demoVideoCaption=q<HTMLElement>("#demo-video-caption");let demoIndices:Record<string,number>={images:0,text:0,video:0};
function renderDemoItem(kind:string){if(kind==="images"){const x=DEMO_IMAGES[demoIndices.images%DEMO_IMAGES.length];demoImage.src=x.src;demoImage.alt=x.caption;demoImageCaption.textContent=x.caption;}else if(kind==="text"){const x=DEMO_TEXTS[demoIndices.text%DEMO_TEXTS.length];demoReading.innerHTML=`<h3>${x.title}</h3><p class="stimulus-source">${x.source} · public-domain text</p>${x.body.map(p=>`<p>${p}</p>`).join("")}`;}else{const x=DEMO_VIDEOS[demoIndices.video%DEMO_VIDEOS.length];demoVideo.pause();demoVideo.src=x.src;demoVideoCaption.textContent=x.caption;demoVideo.load();}}
document.querySelectorAll<HTMLButtonElement>(".stimulus-prev,.stimulus-next").forEach(btn=>btn.onclick=()=>{const panel=btn.closest<HTMLElement>(".stimulus-panel"),kind=panel?.dataset.panel??"images",items=kind==="images"?DEMO_IMAGES:kind==="text"?DEMO_TEXTS:DEMO_VIDEOS,delta=btn.classList.contains("stimulus-next")?1:-1;demoIndices[kind]=(demoIndices[kind]+delta+items.length)%items.length;renderDemoItem(kind);tracker.mark(`demo_${kind}_${demoIndices[kind]+1}`);});renderDemoItem("images");renderDemoItem("text");renderDemoItem("video");
...
useSavedCalibration.onclick=()=>{const saved=readSavedCalibration();if(!saved)return;try{calibration.restoreSavedCalibration(saved);const summary=calibration.calibrationSummary;if(summary)tracker.setCalibrationSummary(summary);showSavedCalibration();validateButton.disabled=false;demoButton.disabled=false;status.textContent="Saved calibration restored. Run validation to check accuracy before recording.";setStep("validation");}catch(e){status.textContent=e instanceof Error?e.message:String(e);}};
